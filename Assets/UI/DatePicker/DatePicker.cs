...
#pragma warning restore
            ...
            foreach (var day in days)
            {
                ...
                if (dateRangeValid) // if the date range is not valid, then don't attempt to use it
                {
                    if ((Config.DateRange.RestrictFromDate && day.CompareTo(Config.DateRange.FromDate) < 0)
                        || (Config.DateRange.RestrictToDate && day.CompareTo(DateTime.Today) > 0))
                    {
        ...
        void UpdateHeader()
        {
            ...
            if (dateRangeValid && Config.DateRange.RestrictToDate)
            {
                ...
                Ref_Header.NextMonthButton.Button.interactable = (firstDayOfNextMonth.CompareTo(DateTime.Today) <= 0);
                ...
                Ref_Header.NextYearButton.Button.interactable = (firstDayOfMonthInNextYear.CompareTo(DateTime.Today) <= 0);
            }
            else
            {
